...
int main()
{
	// inicializa a classe da memória
	...
	while (true)
	{
		...
		// aimbot fov (Field of View = Campo onde o aimbot trabalhara) 
		auto bestFov = 5.f;
		...
		for (auto i = 1; i <= 32; ++i)
		{
			...
			if (memory.Read<std::int32_t>(player + offset::m_bSpottedByMask) & (1 << localPlayerId))
			{
				...
				// Calcula o aimbot de forma 3D, buscara o inimigo mais próximo para evitar conflitos e bugs
		...
		// Calibra o angulo do aimbot para evitar bugs
		if (!bestAngle.IsZero())
			memory.Write<Vector3>(clientState + offset::dwClientState_ViewAngles, viewAngles + bestAngle / 3.f); // suavização do aimbot 
	}
